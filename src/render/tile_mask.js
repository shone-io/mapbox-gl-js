...
module.exports = function(renderableTiles: Array<Tile>, gl: WebGLRenderingContext) {
    ...
    for (let i = 0; i < sortedRenderables.length; i++) {
        ...
        computeTileMasks(tile.tileID.wrapped(), tile.tileID, childArray, new OverscaledTileID(0, tile.tileID.wrap + 1, new CanonicalTileID(0, 0, 0)), mask);
...
function computeTileMasks(rootTile: OverscaledTileID, ref: OverscaledTileID, childArray: Array<Tile>, lowerBound: OverscaledTileID, mask: Mask) {
    ...
    const maskTileId = new OverscaledTileID(diffZ, 0, new CanonicalTileID(diffZ, ref.canonical.x - (rootTile.canonical.x << diffZ), ref.canonical.y - (rootTile.canonical.y << diffZ)));
    mask[maskTileId.id] = mask[maskTileId.id] || maskTileId;
}
